...
const ManagerManagement = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    username: '',
    password: '',
    role: 'manager' // Fixed to manager only
  });
  ...
  const fetchManagers = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({ type: 'error', text: data.message || "Failed to fetch managers" });
      }
    } catch (err) {
      setMessage({ type: 'error', text: "Network error while fetching managers" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const addManager = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({ type: 'error', text: data.message || 'Failed to create manager' });
      }
    } catch (err) {
      setMessage({ type: 'error', text: 'Network error while creating manager' });
    } finally {
      setLoading(false);
    }
  };

  const deleteManager = async (userId, userName) => {
    if (!confirm(`Are you sure you want to delete ${userName} (Manager)?`)) return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({ type: 'error', text: data.message || 'Failed to delete manager' });
      }
    } catch (err) {
      setMessage({ type: 'error', text: 'Network error while deleting manager' });
    } finally {
      setLoading(false);
    }
  };
          ...
          {showAddForm && (
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <h2 className="text-lg font-semibold text-gray-900">Add New Manager</h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2 flex space-x-3 pt-4">
                  ...
                  >
                    ...
                    <span>{loading ? 'Creating...' : 'Create Manager'}</span>
                  </button>
                    ...
                    {managers.map((manager) => (
                      <tr key={manager._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <button
                            onClick={() => deleteManager(manager._id, manager.name)}
                            disabled={loading}
