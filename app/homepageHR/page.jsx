...
export default function Homepage() {
  ...
  const sidebarItems = [
    ...
    { label: "View Reports", icon: "📊", route: "/reports" }, // This will be handled below
  ];
          ...
          <nav className="flex flex-col gap-3 w-full items-center">
            {sidebarItems.map((item) => (
              <button
                ...
                onClick={() => {
                  if (item.label === "View Reports") {
                    router.push("/ReportHR");
                  } else if (item.label === "View Attendance") {
                    router.push("/viewattendanceHR");
                  } else if (item.route) {
